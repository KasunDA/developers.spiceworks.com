...
// basic init for javascript toolkit
window.$UI = {};
...
// create the console and console.log objects if they don't exist to prevent clientside errors
if(typeof(console) == "undefined" || console == null){window.console = {};}
if(typeof(console.log) == "undefined" || console.log == null){window.console.log = function(){}};
if(typeof(console.debug) == "undefined" || console.debug == null){window.console.debug = function(){}};
if(typeof(console.error) == "undefined" || console.error == null){window.console.error = function(){}};
...
// in case the user has resized their window
window.$UI.mobileView = function() {
  return jQuery('.global-nav_mobile-nav-toggle').is(':visible');
}

window.$UI.touchEnabled = function() { return Modernizr.touch };

jQuery(function() {
  if( window.$UI.mobileView() ) {
    function hideiOSAddressBar() {
      if( jQuery(window).scrollTop() < 2 ) { // Only auto-scroll if user is still at the top
        window.scrollTo( 0, jQuery('.mobile-search').outerHeight() );
      }
    }
...
!(function($) {
  var eventNamespace = 'google-analytics';
  $UI.GoogleAnalytics = {
    trackEvent: function( category, action, label, value ) {
      $(document).trigger( eventNamespace + ':send:event', [category, action, label, value] );
    }
  };
})(jQuery);
...
!function($) {
  ...
  Navbar.prototype = {
    ...
    enableMenuFade: function() {
      ...
      elements.on( 'hidden.bs.dropdown', function() {
        $(this).find('[data-content="nav-dropdown"]').removeClass('u-faded-in');
      });
      elements.on( 'shown.bs.dropdown', function() {
  ...
  var GlobalNavbar = function() {
    ...
    this.searchAutocompleteMinLength = this.$el.data('search-autocomplete-min-length');
  ...
  GlobalNavbar.prototype = $.extend( new Navbar(), {
    ...
    initSearch: function() {
      ...
      if( this.$searchMenu.length ) {
        ...
        this.searchRibbon = new $UI.SearchRibbon({ nav_height: globalNavHeight,
                                                   autocompleteMinLength: this.searchAutocompleteMinLength });
      }
    },

    toggleGlobalSearch: function(event) {
      event.stopPropagation();
      event.preventDefault();
      $(document).trigger( 'global-search:' + (this.$searchMenu.hasClass('open') ? 'hide' : 'show') );
    },

    openSearchMenu: function() { // Just clears other dropdowns and shows the caret 
      $.fn.dropdown.Constructor.prototype.clearMenus();
      this.$searchMenu.addClass('open');
      var reflow = $('body').offset().left; // force reflow
      this.$searchMenu.find('[data-content="nav-dropdown"]').addClass('u-faded-in');
    },

    closeSearchMenu: function() {
      this.$searchMenu.removeClass('open in');
    },
    ...
    showMobileNav: function() {
      $('body').addClass( this.mobileNavOpenBodyClass() );
    },

    hideMobileNav: function() {
      $('body').removeClass( this.mobileNavOpenBodyClass() );
    },

    mobileNavOpenBodyClass: function() {
      return Modernizr.csstransforms ? 'mobile-nav-open' : 'mobile-nav-open-compat';
    }
  ...
  var DomesticNavbar = function() {
    Navbar.apply(this, arguments);

    $('body').addClass('double-navbar');
  ...
  DomesticNavbar.prototype = $.extend( new Navbar(), {
    ...
    makeSticky: function() {
      $('nav.domestic-nav').affix({ offset: {top: globalNavHeight} });
    },

    ////////////////////////////////////////////////////////////////////////////////////////////////////
    // Override these three for your own behavior when building the navbar with JavaScript
    ////////////////////////////////////////////////////////////////////////////////////////////////////
    urlFor:     function(menuItem) { return menuItem.url; },  // use 'url' in the menuItem definition
    isActiveMenu: function(menuItem) { return null; }, // null means look for 'active' in the menuItem definition
    onRoute:    function(menuItem) { },              // Do nothing by default
    ////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    configure: function(contents) {
      ...
      this.brandTemplate          = _.template('<a class="domestic-nav_brand" href="<%= url %>"><div class="swoosh-logo"></div><span class="domestic-nav_title"><%= text %></span>');
      this.menusTemplate          = _.template('<ul class="domestic-menus"></ul>');
      this.secondaryMenusTemplate = _.template('<ul class="secondary-menus"></ul>');
      this.menuItemTemplate       = _.template('<li class=" domestic-menus_entry <%= classNames %>"><a href="<%= url %>"><% if( typeof(icon) != "undefined" ) { %><i class="icon-<%= icon %> icon-light"></i><% } %><%= text %></a></li>');
      this.menuDropdownTemplate   = _.template('<li class=" domestic-menus_entry domestic-menus_dropmenu <%= classNames %>"><a href="javascript:void(0)" data-toggle="dropdown"><% if( typeof(icon) != "undefined" ) { %><i class="icon-<%= icon %> icon-light"></i><% } %><%= text %></a><ul class="domestic-menus_submenu domestic-menus_submenu_items"></ul></li>');
    ...
    _menuClicked: function(menuItem, event) {
      event.preventDefault();
      this.onRoute(menuItem);
    },

    _itemIsActive: function(menuItem) {
      var isActive = this.isActiveMenu(menuItem);
  ...
  $UI.DomesticNavbar = DomesticNavbar;
...
!function($) {
  ...
  $UI.SearchRibbon = function(options) {
    // Don't want to do a lot of work here since most page loads won't see search get used
    this.options = _.defaults( options, $UI.SearchRibbon.defaults );
    $(document).on( 'global-search:show', $.proxy( this.show, this ) );
  }

  $UI.SearchRibbon.defaults = {
    autocompleteMinLength: 2
  };

  $UI.SearchRibbon.prototype = {
    ...
    init: function() {
      ...
      this.$el = $( '<div class="search-ribbon sui-opt-in u-fade-able"></div>' );
      $('body').append(this.$el);

      this.renderRibbon();

      $(document).on( 'global-search:hide', $.proxy( this.hide, this ) );
      $(document).on( 'click', $.proxy( this.hideOnClickOut, this ) );

      var nextLetterWaitTimeout = 200;
      this.debouncedAutocomlete = _.debounce( $.proxy( this.autocomplete, this, this.query ), nextLetterWaitTimeout );
    ...
    show: function() {
      ...
      this.$el.show();
      var reflow = $('body').offset().left; // force reflow
      this.$el.addClass('u-faded-in');

      this.setVerticalPositioning();
      $(window).on( 'resize.search-ribbon.vertical-positioning', $.proxy( this.setVerticalPositioning, this ) );

      this.$el.affix({ offset: {top: this.options.nav_height} });
      this.$el.affix('checkPosition'); // Hiding search ribbon confuses affix plugin. This wakes it up.
      this.$input.focus();
      ...
      $(document).trigger( 'global-search.shown' );

      this.$el.on( 'click.global-search.analytics-capture', '.search-ribbon_result > a', _.bind( function(event) {
        this.analyticsForSelection( $(event.target).parent() );
      }, this ) );

      $UI.GoogleAnalytics.trackEvent('QuickNav', 'Opened');
    },
    ...
    hide: function() {
      if( this.$el && this.$el.is(':visible') ) {
        ...
        $(window).off( 'resize.search-ribbon.vertical-positioning' );
        this.$el.off( 'click.global-search.analytics-capture' );

        if( $.support.transition && $.support.transition.end ) {
          // This doesn't work right b/c of animation on input highlight change.
          // see setTimeout hack below for now
          //   this.$el.one( $.support.transition.end, $.proxy( this.$el.hide, this.$el ) );
          this.$el.removeClass('u-faded-in');
        }
        else {
          this.$el.hide();
        }
        ...
        $(document).trigger( 'global-search.hidden' );

        var _this = this;
        setTimeout( function() {
          _this.$el.hide();
          _this.$el.affix('checkPosition'); // Hiding search ribbon confuses affix plugin. This wakes it up.
        }, 150 );
      }
    },
    ...
    renderRibbon: function() {
      ...
      this.$el.find('form').off( 'submit.search-ribbon.full-search' );
      this.$el.find('.js-app-selection-list a').off( 'click.search-ribbon.app-select' );

      var templateData = {
        selected_app_icon: this.selectedAppName === 'community' ? 'community-notifications-icon' : 'apps-menu-icon',
        placeholder: this.selectedAppName === 'community' ? 'Search the community' : 'Search inventory and helpdesk',
        apps_config_class: _.keys( $UI.appInterfaces.apps ).length === 1 ? 'search-ribbon_single-app' : ''
      };

      this.$el.html( this.ribbonTemplate( templateData ) );

      this.$input = this.$el.find('input[type="text"]');
      this.$spinnerEl = this.$el.find('.search-ribbon_spinner');
      this.$resultsEl = $( this.resultsTemplate() );
      this.$suggestionsEl = this.$resultsEl.find('[data-content="search-suggestions"]');
      this.$controlsEl = this.$el.find('.search-ribbon_controls');

      this.$controlsEl.append( this.$resultsEl );
      ...
      this.$el.find('form').on( 'submit.search-ribbon.full-search', $.proxy( this.submitFullSearch, this ) );
      appSelectionList.find('a').on( 'click.search-ribbon.app-select', $.proxy( this.newAppSelected, this ) );
    },
    ...
    setVerticalPositioning: function() {
      var menuLeft = $('[data-menu-type="search"]').offset().left; // Find the search menu in the global nav... so ugly, I'm sorry

      var buttonLeft = this.$controlsEl.find('button').offset().left;
      var searchLeft = this.$controlsEl.offset().left;
      var pushLeft = buttonLeft-searchLeft;

      var pushRight = 10; // line up w/ left edge of notch not the actual menu's left edge

      this.$controlsEl.css( 'left', menuLeft-pushLeft+pushRight+'px' );
    },

    hideOnClickOut: function(event) {
      if( $(event.target).closest('.search-ribbon').length === 0 ) {
        this.hide();
      }
    },
    ...
    autocompleteIfLongEnough: function() {
      ...
      if( this.query.length >= this.options.autocompleteMinLength ) {
    ...
    showAutocompletePanel: function() {
      ...
      exactResult.find('.search-ribbon_result-name').html("'" + this.query + "'");
    ...
    hideAutocompletePanel: function() {
      if( this.$resultsEl ) {
        this.$resultsEl.hide();
        this.$resultsEl.find('.search-ribbon_groups').addClass('u-collapsed');
      }
    ...
    insertResultGroups: function(result_groups) {
      ...
      var groups = this.$resultsEl.find('.search-ribbon_groups');

      groups.find('.search-ribbon_group').remove();
      ...
      _.each( result_groups, function( group ) {
        ...
        var groupResults = groupEl.find('.search-ribbon_group-results');

        _.each( group.results || [], function( result ) {
          var data = _.extend( { name: '', excerpt: '', path: '#' }, result );
      ...
      if( result_groups.length ) {
        groups.removeClass('u-collapsed');
      }
      else {
        groups.addClass('u-collapsed'); // Hide the groups list if there are no results
      }
    },
    ...
    fadeOutResults: function() { 
        this.$resultsEl.find('.search-ribbon_group').removeClass('u-faded-in').addClass('u-faded-almost-out');
      },

    fadeInResults: function() { 
        this.$resultsEl.find('.search-ribbon_group').removeClass('u-faded-almost-out').addClass('u-faded-in');
    },
    ...
    selectedResult: function() {
      return this.$resultsEl.find('.search-ribbon_result.is-selected');
    },

    resultElements: function() {
      return this.$resultsEl.find('.search-ribbon_result');
    },
    ...
    analyticsForSelection: function(result) {
      ...
      var group = result ? result.parents('.search-ribbon_group') : null;

      if( result && group.length ) {
        var type = group.find('.search-ribbon_group-name').text();
        var name = result.find('.search-ribbon_result-name').text();
        $UI.GoogleAnalytics.trackEvent('QuickNav', 'Item selected', name + ' - ' + type + ' - "' + this.query + '"');
      }
      else {
        $UI.GoogleAnalytics.trackEvent('QuickNav', 'Item selected', 'Full search - Search - "' + this.query + '"');
      }
    },
  ...
  $(document).keydown(function(event) {
    ...
    if( keyCode === searchKey ) {
      event.preventDefault();
      $(document).trigger( 'global-search:show');
  ...
  $(document).keydown(function(event) {
    ...
    if(key === escapeKey) {
      $(document).trigger( 'global-search:hide');
    }
  });
...
ZeroClipboard.config( {
  swfPath: "//spiceworks.github.io/developers.spiceworks.com/images/zeroclipboard/ZeroClipboard.swf" } );
